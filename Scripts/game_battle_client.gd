...
# Client-specific initialization and logic
...
func _ready() -> void:
	...
	print("[GameBattle CLIENT] My peer ID: ", my_peer_id)
	print("[GameBattle CLIENT] All connected peers: ", peers)
	print("[GameBattle CLIENT] Am I server? ", multiplayer.is_server())
	
	if peers.size() > 0:
		opponent_peer_id = peers[0]
		print("[GameBattle CLIENT] Opponent peer ID: ", opponent_peer_id)
	else:
		print("[GameBattle CLIENT] ERROR: No other peers found!")
		return
	...
	# CLIENT-SPECIFIC: Tell the server this peer is ready
	report_peer_ready.rpc_id(1, my_peer_id)
	
	# CLIENT-SPECIFIC: Client waits for battle to start
	print("[GameBattle CLIENT] CLIENT: I am NOT the server, waiting for battle to start...")
