...
func _ready() -> void:
	...
	# Get opponent peer ID - the OTHER peer connected to this multiplayer session
	var peers = multiplayer.get_peers()
	print("[GameBattle] My peer ID: ", my_peer_id)
	print("[GameBattle] All connected peers: ", peers)
	print("[GameBattle] Am I server? ", multiplayer.is_server())
	
	if peers.size() > 0:
		opponent_peer_id = peers[0]
		print("[GameBattle] Opponent peer ID: ", opponent_peer_id)
	else:
		print("[GameBattle] ERROR: No other peers found!")
		return
	...
	# Tell the server this peer is ready
	report_peer_ready.rpc_id(1, my_peer_id)
	
	# ONLY the server waits for both peers to be ready
	if multiplayer.is_server():
		print("[GameBattle] SERVER: I am the server, waiting for both peers...")
		...
		if peers_ready.size() >= 2:
			print("[GameBattle] SERVER: Both peers ready! Calling initialize_battle.rpc()")
			initialize_battle.rpc()
		else:
			print("[GameBattle] SERVER: ERROR: Not all peers ready after timeout! Ready: ", peers_ready.size())
	else:
		print("[GameBattle] CLIENT: I am NOT the server, waiting for battle to start...")
